...
interface BClassTreemapChartProps {
  trades: Trade[];
...
// Build hierarchical data for treemap from trades
function buildTreemapData(
  ...
  hierarchy.forEach((l1Map, l1Name) => {
    const l1Children: TreemapDataItem[] = [];
    let l1Total = { volume: 0, count: 0 };

    l1Map.forEach((l2Map, l2Name) => {
      const l2Children: TreemapDataItem[] = [];
...
export function BClassTreemapChart({
  trades,
  onSegmentClick,
  selectedFilter,
}: BClassTreemapChartProps) {
  const [metric, setMetric] = useState<MetricMode>('volume');

  const treemapData = useMemo(
    () => buildTreemapData(trades, metric),
    [trades, metric]
  );
  ...
  const handleChartClick = useCallback((params: { data?: TreemapDataItem }) => {
  ...
  const option = useMemo(() => {
    return {
      ...
      tooltip: {
        ...
        formatter: (params: { data?: TreemapDataItem }) => {
  ...
  return (
    <div className="treemap-chart-container">
      <div className="treemap-chart-header">
        <div className="treemap-chart-title">
          <h3>Asset Class Breakdown</h3>
          <span className="treemap-chart-total">
